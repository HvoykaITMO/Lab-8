...
FLY_SIZE = 64  # ширина и высота моей мухи (число выбрано исходя из размеров мухи в файле fly64.png


# Принимаем координаты, ширину и высоту каждого прямоугольника и просто проверяем, как их верхние левые и правые
# нижние точки расположены по отношению друг к другу
...
tsokotuha = cv2.imread('fly2.jpg')  # Изображение Цокотухи (я взял своё c более удобным задним фоном)
tsokotuha = cv2.resize(tsokotuha, (FLY_SIZE, FLY_SIZE))  # Меняем размер фото такой же как у мухи из файла fly64
...
while True:
    ...
    if len(contours) > 0:
        ...
        new_fly_x = x - (fly_w - w) // 2  # Полученная формула для расположения Цокотухи в центре контура по оси x
        new_fly_y = y - (fly_h - h) // 2  # Полученная формула для расположения Цокотухи в центре контура по оси y
        roi = frame[new_fly_y:new_fly_y + fly_h, new_fly_x:new_fly_x + fly_w]  # Выбираем интересующую нас область
        if roi.shape == tsokotuha.shape:

            '''Сделаем прозрачность заднего фона у цокотухи'''
            gray_fly = cv2.cvtColor(tsokotuha, cv2.COLOR_BGR2GRAY)  # Серая цокотуха
            thr_fly = cv2.threshold(gray_fly, 215, 255, cv2.THRESH_BINARY_INV)[1]  # Черно-белая
            alpha = thr_fly / 255  # Нормализуем, чтобы значения были в диапазоне от 0 до 1
            alpha_inv = 1 - alpha  # Понадобится для фона
            for c in range(3):  # Преобразуем каждый канал
                '''Преобразуем по простой логике: если наша цокотуха непрозрачна в данном пикселе (alpha = 1),
                то это не фоновый пиксель и alpha_inv = 0, если alpha = 0, то у цокотухи это полностью прозрачный
                пиксель, поэтому поместим на этом пикселе фон, то есть alpha_inv = 1 умножим на пиксель фрейма.
                Так делаем с каждым каналом каждого пикселя и получится, что непрозрачные пиксели будут цвета цокотухи,
                а прозрачные - цвета заднего фона.'''
                roi[:, :, c] = (alpha * tsokotuha[:, :, c] + alpha_inv * roi[:, :, c])

            frame[new_fly_y:new_fly_y + fly_h, new_fly_x:new_fly_x + fly_w] = roi  # Вставим результат
